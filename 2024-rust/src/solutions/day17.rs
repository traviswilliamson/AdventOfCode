...
fn find_a_for_output(index_of_output_to_sieve: i32, a_to_check: u64, program: &Vec<u64>) -> Option<u64> {
    if index_of_output_to_sieve < 0 {
        ...
        for low_digits in 0..8 {
            ...
            if output == program[index_of_output_to_sieve as usize] {
                let bigger_a = find_a_for_output(index_of_output_to_sieve - 1, (a_to_check << 3) | low_digits, &program);
                if bigger_a.is_some() {
...
pub fn second(_input: String) -> Result<String, Box<dyn Error>> {
    // I know, trust me
    let program = _input.trim().split("\n\n").skip(1).next().unwrap().split(": ").skip(1).next().unwrap()
        .split(",").map(|i| i.parse::<u64>().unwrap()).collect::<Vec<u64>>();
    // println!("{:?}", program);
    Ok(find_a_for_output((program.len() - 1) as i32, 0, &program).unwrap().to_string())
}
